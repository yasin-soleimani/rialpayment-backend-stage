export const UnknownDependenciesMessage = (
  ...
  let message = `Vision can't resolve dependencies of the ${type}`;
  message += ` (`;
...
export const InvalidModuleMessage = (scope: string) =>
  `Vision cannot create the module instance. The frequent reason of this exception is the circular dependency between modules. Use forwardRef() to avoid it (read more https://docs.Visionjs.com/advanced/circular-dependency). Scope [${scope}]`;

export const UnknownExportMessage = (module: string) =>
  `Vision cannot export component/module that is not a part of the currently proccessed module (${module}). Please verify whether each exported unit is available in this particular context.`;
...
export const MICROSERVICES_PACKAGE_NOT_FOUND_EXCEPTION = `Unable to load @vision/microservices package (please, make sure whether it's installed already).`;
