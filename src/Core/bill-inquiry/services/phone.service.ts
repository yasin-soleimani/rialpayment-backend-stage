import { faildOpt, Injectable, InternalServerErrorException, successOptWithDataNoValidation } from '@vision/common';
...
import { BillInquiryMCISubmit, BillIqnuiryMCIRequestModel, BillIqnuiryTelRequestModel } from '../function/mobile.func';
import { BillInquiryCommonService } from './common.service';

@Injectable()
export class BillInquiryPhoneCoreService {
  ...
  async getData(id: string): Promise<any> {
    const data = BillIqnuiryTelRequestModel(id);
    return this.client.post('FixedLineBillInquiry', data).then((res) => {
      console.log(res, 'res');
      return res.data;
    });
  }
