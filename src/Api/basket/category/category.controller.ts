...
@Controller('basket/category')
export class BasketCategoryController {
  constructor(
  ...
  async addNew(@Body() getInfo: BasketCategoryApiDto, @Req() req): Promise<any> {
    const userid = await this.generalService.getUserid(req);
    getInfo.user = userid;
    return this.categoryService.addNew(getInfo);
  }
  ...
  async checkSlug(@Body() getInfo: Pick<BasketCategoryApiDto, 'slug' | 'user'>, @Req() req): Promise<any> {
    const userid = await this.generalService.getUserid(req);
    getInfo.user = userid;
    return this.categoryService.checkSlug(getInfo);
  }

  @Get()
  async getList(@Req() req): Promise<any> {
    const userid = await this.generalService.getUserid(req);
    return this.categoryService.getList(userid);
  }

  @Put()
  async udpate(@Body() getInfo: BasketCategoryApiDto, @Req() req): Promise<any> {
    const userid = await this.generalService.getUserid(req);
    return this.categoryService.edit(getInfo, userid);
  }
